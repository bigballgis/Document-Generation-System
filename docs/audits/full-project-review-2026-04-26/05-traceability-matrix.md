...
| DIFF-TEST-001 | docx content diff checked tasks need real tests | `.kiro/specs/docx-content-diff/tasks.md` | `ContentDiffService`, `DocxTextExtractor`, `SegmentVersionService` | `ContentDiffServiceTest` (WS-04-T01); `DocxTextExtractor` / `SegmentVersionService` tests pending WS-04-T02+ | `docs/segment-version-api.md` to update | In Progress |
