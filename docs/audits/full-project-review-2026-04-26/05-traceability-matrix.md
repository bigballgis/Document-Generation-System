...
| IMPORT-ZIP-001 | ZIP imports need size, count, ratio, and path limits | Composite import/export spec to update | `CompositeImportExportService` | `CompositeImportExportServiceTest` (WS-03-T01 boundary characterization: invalid bytes, empty archive, malformed config, ignored paths, zip-slip-style names, large entry, many entries) | TBD | Open |
